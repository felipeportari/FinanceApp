...
@section('title', __('app.admin.create_user'))
@section('page-title', __('app.admin.create_user'))
@section('page-subtitle', __('app.admin.create_subtitle'))
...
<div class="max-w-2xl mx-auto">
    <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-6">
        ...
        <form method="POST" action="{{ route('admin.users.store') }}" class="space-y-5">
            ...
            {{-- Password --}}
            <div>
                <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">{{ __('app.admin.password_label') }}</label>
            ...
            <div>
                ...
                <select name="locale" required
                        class="w-full px-3 py-2.5 rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm">
                    <option value="pt" {{ old('locale', 'pt') === 'pt' ? 'selected' : '' }}>{{ __('app.settings.lang_pt') }}</option>
                    <option value="en" {{ old('locale') === 'en' ? 'selected' : '' }}>{{ __('app.settings.lang_en') }}</option>
                    <option value="es" {{ old('locale') === 'es' ? 'selected' : '' }}>{{ __('app.settings.lang_es') }}</option>
                </select>
                @error('locale')
                    <p class="mt-1 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
